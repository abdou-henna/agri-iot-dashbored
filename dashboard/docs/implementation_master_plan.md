...
Production deployment executes at **Phase 9**.

## Phase 9–12 — Compressed Agronomic Intelligence Roadmap

Current status: the existing Phase 9–12 runtime implementation at commit `d87f11f` is a prototype/skeleton and is not production-final.

Baseline documents required before any further runtime patching:
...
Execution strategy (3 stages only):
1. Planning baseline (this documentation set)
2. Deterministic logic refinement
3. Gemini and UI refinement

No additional Phase 9–12 runtime code patch should proceed until baseline docs exist and are acknowledged.

Promotion acceptance criteria to production-quality:
- Deterministic contracts satisfied for farm state, irrigation, cutting/regrowth, fertilization, pivot intelligence, and deferred predictive risk signaling.
- Reliability and confidence are evidence-based and pivot-weighted where applicable.
- Missingness and limitations are explicit in deterministic payload and UI.
