...
export function ProcessedDataViewerTable({ model }: { model: ProcessedDataViewerModel }) {
  ...
  const rowView = (row: ProcessedDataViewerRow) => (<>
    <td>{row.domain}</td><td>{row.source_label}</td><td>{provenanceLabel(row.provenance_source)}</td><td>{row.metric}</td><td>{valueLabel(row.raw_value)}</td><td>{valueLabel(row.cleaned_value)}</td><td>{valueLabel(row.processed_value)}</td>
  ...
  return <div className="space-y-3">
    ...
    <div className="hidden overflow-auto md:block"><table className="min-w-full text-xs"><thead><tr>{['domain','source/node','provenance','metric','raw','cleaned','processed','status','QC','reliability','alerts','timestamp','limitations'].map((h) => <th key={h} className="px-2 py-1 text-left">{h}</th>)}</tr></thead><tbody>{filteredRows.map((row) => <tr key={row.id} className="border-t">{rowView(row)}</tr>)}</tbody></table></div>
    <div className="space-y-2 md:hidden">{filteredRows.map((row) => <div key={row.id} className="rounded border p-2 text-xs"><div className="font-semibold">{row.metric} · {row.source_label}</div><div>Provenance: {provenanceLabel(row.provenance_source)}</div><div className="mt-1">Status: <StatusBadge status={row.processing_status} /></div><div>Raw: {valueLabel(row.raw_value)} | Cleaned: {valueLabel(row.cleaned_value)} | Processed: {valueLabel(row.processed_value)}</div><div>Limitations: {row.limitations.join(' | ')}</div></div>)}</div>
  </div>;
}
